...
const ShoppingListView: React.FC<ShoppingListViewProps> = ({ items, onToggle, onUpdateQuantity, onClearCompleted, onClearAll, onToggleAll }) => {
  ...
  const handleShareListClick = () => {
    if (items.length === 0) {
      alert('¡Tu lista de la compra está vacía! Añade algunos artículos para poder compartirla.');
      return;
    }
    setIsShareOptionsModalOpen(true);
  };

  const executeShare = async (shareMode: 'all' | 'checked' | 'unchecked') => {
    setIsShareOptionsModalOpen(false);

    let itemsToShare: ShoppingListItem[] = [];
    let title = 'Lista de la Compra de Homsent Chef';
    let textBody: string;

    if (shareMode === 'checked') {
        itemsToShare = items.filter(item => item.checked);
        title = 'Ingredientes Comprados';
    } else if (shareMode === 'unchecked') {
        itemsToShare = items.filter(item => !item.checked);
        title = 'Ingredientes Pendientes';
    } else {
        itemsToShare = items;
    }

    if (itemsToShare.length === 0) {
        let message = 'No hay ingredientes para compartir con esta opción.';
        if (shareMode === 'checked') message = 'No has seleccionado ningún ingrediente.';
        if (shareMode === 'unchecked') message = '¡Has completado toda tu lista!';
        alert(message);
        return;
    }

    if (shareMode === 'all') {
        textBody = items.map(item => `${item.checked ? '✅' : '◻️'} ${item.name.trim()} (${item.quantity.trim()})`).join('\n');
    } else {
        textBody = itemsToShare.map(item => `- ${item.name.trim()} (${item.quantity.trim()})`).join('\n');
    }
  ...
  const SearchModal = () => (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setIsSearchModalOpen(false)}>
        <div className="bg-gray-800 border border-[#D4AF37]/50 rounded-xl shadow-2xl p-6 w-full max-w-lg animate-modal-in" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-center text-gray-400 mb-6">Selecciona tu supermercado para buscar tus ingredientes pendientes.</p>
  ...
  const ShareOptionsModal = () => (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[51] p-4" onClick={() => setIsShareOptionsModalOpen(false)}>
        <div className="bg-gray-800 border border-[#D4AF37]/50 rounded-xl shadow-2xl p-6 w-full max-w-md animate-modal-in" onClick={e => e.stopPropagation()}>
            <h3 className="text-2xl font-bold text-center text-white mb-2">¿Qué quieres compartir?</h3>
            <p className="text-center text-gray-400 mb-6">Elige qué parte de tu lista quieres enviar.</p>
            
            <div className="space-y-3">
                <button
                    onClick={() => executeShare('checked')}
                    className="w-full text-left p-3 bg-gray-700 hover:bg-[#D4AF37]/10 rounded-lg border-2 border-transparent hover:border-[#D4AF37]/50 transition-all duration-300"
                >
                    <p className="font-semibold text-white">Solo los ingredientes comprados (✅)</p>
                    <p className="text-sm text-gray-400">Envía los artículos que ya has marcado.</p>
                </button>
                 <button
                    onClick={() => executeShare('unchecked')}
                    className="w-full text-left p-3 bg-gray-700 hover:bg-[#D4AF37]/10 rounded-lg border-2 border-transparent hover:border-[#D4AF37]/50 transition-all duration-300"
                >
                    <p className="font-semibold text-white">Solo los ingredientes pendientes (◻️)</p>
                    <p className="text-sm text-gray-400">Ideal para pedir a alguien que compre lo que falta.</p>
                </button>
                 <button
                    onClick={() => executeShare('all')}
                    className="w-full text-left p-3 bg-gray-700 hover:bg-[#D4AF37]/10 rounded-lg border-2 border-transparent hover:border-[#D4AF37]/50 transition-all duration-300"
                >
                    <p className="font-semibold text-white">La lista completa</p>
                    <p className="text-sm text-gray-400">Muestra todos los artículos y su estado actual.</p>
                </button>
            </div>

            <button onClick={() => setIsShareOptionsModalOpen(false)} className="mt-6 w-full bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg transition-colors">
                Cancelar
            </button>
        </div>
    </div>
  ...
  return (
    <>
      ...
      {isShareOptionsModalOpen && <ShareOptionsModal />}
