...
import { identifyIngredientsFromImage, generateRecipe, searchGlobalRecipes, findRecipeVideo } from './services/geminiService';
import type { Recipe, FilterOption, ShoppingListItem } from './types';
...
const App: React.FC = () => {
  ...
  const handleGlobalSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      // Now, find videos for each result
  ...
  const handleToggleGlobalFavorite = useCallback((recipeToToggle: Recipe) => {
    setAllRecipes(prevRecipes => {
      ...
      if (existingRecipeIndex !== -1) {
        // Recipe with same title exists, just toggle its favorite status
        ...
        // New recipe, add it as a favorite with 'Mundial' origin
        const newFavoriteRecipe: Recipe = {
          ...recipeToToggle,
          id: `recipe-${new Date().getTime()}`, // Give it a new, persistent ID
          favorito: true,
          origen: 'Mundial', // Tag it as "world recipe"
        };
          ...
          <div className="w-full animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row gap-4 mt-6">
              ...
              <button
                ...
                className="w-full md:flex-1 bg-[#D4AF37] hover:bg-[#c09d2e] text-black font-bold py-3 px-6 rounded-lg shadow-lg hover:shadow-[#D4AF37]/50 transform hover:scale-105 transition-all duration-300"
              >
                Crear Otra Receta
              </button>
            </div>
